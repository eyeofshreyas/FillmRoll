...
async function openModal(data) {
  ...
  try {
    const res = await fetch('/movie-details', {
      ...
      body: JSON.stringify({ movie_id: data.movie_id, title: data.title }),
    });
    ...
    if (det.trailer_key) {
      tw.innerHTML = `<iframe src="https://www.youtube.com/embed/${det.trailer_key}?autoplay=1&rel=0"
...
function renderFeatured(sel, results) {
  ...
  $('sel-overview').textContent =
    sel && sel.overview
      ? sel.overview
      : `Showing ${results.length} films similar to "${title}".`;
...
function renderGrid(results) {
  ...
  $('rec-count').textContent = `${results.length} films`;
